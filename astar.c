...
int my_evaluator(int *board, int *goal);
...
int score(int *board, int *goal, int depth) {
  switch (eval_func) {
  case EVAL_FAIR:
    return fair_evaluator(board, goal) + depth;
  case EVAL_WEAK:
    return weak_evaluator(board, goal) + depth;
  case EVAL_MY:
    return my_evaluator(board, goal) + depth;
  default:
    return bad_evaluator(board, goal) + depth;
  }
}
...
int astar_empty() { return PQ_Size() == 0; }
/* Set board state as "visited" */
void set_visited(int *board) { Hash_Insert(board2val(board)); }
/* Is board state "visited"? */
int is_visited(int *board) { return Hash_Search(board2val(board)); }
...
int main(int argc, char *argv[]) {
  ...
  fscanf(fp, "%d %d %d %d %d %d %d %d %d", &init[0], &init[1], &init[2],
         &init[3], &init[4], &init[5], &init[6], &init[7], &init[8]);
  fclose(fp);

  printf("[f]	Fair Evaluator\n");
  printf("[w]	Weak Evaluator\n");
  printf("[b]	Bad Evaluator\n");
  printf("[m]	My Evaluator\n");
  printf("[e]	Exit\n");
  for (;;) {
    scanf(" %c", &command);
    switch (command) {
...
// ver 2
//
int calculate_manhattan_distance(int *board, int *goal) {
...
int calculate_linear_conflicts(int *board, int *goal) {
  int linear_conflicts = 0;
  int goal_positions[9] = {0};
  int i, j;
  ...
  for (i = 0; i < 9; ++i) {
    if (board[i] != 0 && board[i] != goal[i]) {
      for (j = i + 1; j < 9; ++j) {
        if (board[j] != 0 && board[j] != goal[j] &&
            goal_positions[board[i]] > goal_positions[board[j]]) {
          ++linear_conflicts;
        }
      }
    }
...
int my_evaluator(int *board, int *goal) {
  return calculate_manhattan_distance(board, goal) +
         2 * calculate_linear_conflicts(board, goal);
}
...
void downheap(int n) {
  for (;;) {
    ...
    if (c >= Heap_Size)
      break;
    if (c + 1 < Heap_Size && Heap[c].score > Heap[c + 1].score)
      c++;
    if (Heap[n].score <= Heap[c].score)
      break;
...
void upheap(int n) {
  for (;;) {
    struct heapentry tmp;
    int p = (n - 1) / 2;
    if (p < 0 || Heap[p].score <= Heap[n].score)
      break;
    memcpy(&tmp, &Heap[n], sizeof(struct heapentry));
...
void PQ_Init() { Heap_Size = 0; }
int PQ_Size() { return Heap_Size; }
void PQ_Push(long val, int depth, int score) {
